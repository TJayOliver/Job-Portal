...
const Subscribers = () => {
    ...
    useEffect( () => {
        ...
        fetch('subscriber', setRetrievedSubscriberData, setLoading, signal, setMessage);
        fetch('subscriber/notify', setNotificationMessages, setLoading, signal, setMessage);
    ...
    const currentSubscribersList = retrievedSubscriberData.slice(firstPostIndex, lastPostIndex); 
    const currentNotificationMessages = notificationMessages.slice(firstPostIndex, lastPostIndex); 
    ...
    const submitMail = (e) => {
        e.preventDefault();
        axios.post('http://locahost:4040/subscribe/notify', mailContent);
    }
    ...
    return (
        <>
            ...
            <section className=" md:ml-64 relative">
                <FormsDashboardHead title='Subscribers' />

                {/* buttons */}
                <div className="p-3 flex justify-between">
                    {sendMail ? null :
                        <div role='button' onClick={ handleSendMail } className={'border border-gray-50 bg-teal-500 w-56 text-white font-medium rounded-md p-3 text-center'}>
                            <p>Send Mail to Subscribers</p> 
                        </div> 
                    }

                    {sendMail ? 
                        <div role='button' onClick={ cancel } className={ 'border border-gray-50 bg-red-400 w-20 text-white font-medium rounded-md p-3 text-center'}>
                            <p>Cancel</p> 
                        </div> : null
                    }

                    {viewMessages ?  
                        <div role='button' onClick={ handleSubscribersList } className={ sendMail ? 'hidden' :'border border-gray-50 bg-green-500 w-56 text-white font-medium rounded-md p-3'}>
                            <p className="text-center">View Subscribers</p>
                        </div> 
                        : 
                        <div role='button' onClick={ handleViewMessages } className={ sendMail ? 'hidden' :'border border-gray-50 bg-blue-500 w-56 text-white font-medium rounded-md p-3'}>
                    ...
                    <div className=" p-3" >
                        <div className=" w-full m-auto p-2 mt-2 border border-gray-200 rounded-md ">
                            <form onSubmit={submitMail} className="flex flex-col gap-4">
                                ...
                                <button 
                                    type="submit" 
                                    className="bg-teal-700 p-3 rounded-md text-white font-medium">
                                    Send
                                </button>
                        ...
                        <div className=" border border-gray-200 rounded-md">
                            <table className=" min-w-full divide-y divide-gray-200">
                                <thead className=" bg-gray-50">
                                    <tr>
                                        <th className="px-2 md:px-4 py-3 text-left text-sm font-medium flex gap-1" >Subscribers <CgArrowDown className="mt-1"/></th>
                                ...
                                <tbody className="divide-y divide-gray-200">
                                    {
                                        ...
                                        currentSubscribersList.map( data => (
                                            <tr key={data.id} className=" hover:bg-gray-50">
                                                <td className="px-2 md:px-4 py-4 text-left text-xs font-medium">{data.email}</td>
                    ...
                    <div className="p-2">
                        <div className=" border border-gray-200 rounded-md">
                            <table className=" min-w-full divide-y divide-gray-200">
                                <thead className=" bg-gray-50">
                                    <tr>
                                        <th className="px-2 md:px-4 py-3 text-left text-sm font-medium">Subject</th>
                                        <th className="px-2 md:px-4 py-3 text-left text-sm font-medium">Date Sent</th>
                                        <th className="px-2 md:px-4 py-3 text-left text-sm font-medium" ></th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-gray-200">
                                    {
                                        ...
                                        currentNotificationMessages.map( data => (
                                            <tr key={data.id} className=" hover:bg-gray-50">
                                                <td className="px-2 md:px-4 py-4 text-left text-xs font-medium">{data.subject}</td>
